...
impl ArchCodegen for ArmCodegen {
    ...
    // ---- float binops ----
    fn emit_float_binop(&mut self, dest: &Value, op: crate::backend::cast::FloatOp, lhs: &Operand, rhs: &Operand, ty: IrType) { self.emit_float_binop_impl(dest, op, lhs, rhs, ty) }
